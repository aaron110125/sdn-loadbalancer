...
FLOW_MEMORY_TIMEOUT = 60 * 5                                  #flow control
...
class MemoryEntry (object):
  ...
  def __init__ (self, server, first_packet, client_port):
    self.server = server                                              #server control
    self.first_packet = first_packet
    self.client_port = client_port
    self.refresh()

  def refresh (self):
    self.timeout = time.time() + FLOW_MEMORY_TIMEOUT                  #for timeout
...
def launch (ip, servers, dpid = None):
  ...
  core.openflow.addListenerByName("ConnectionUp", _handle_ConnectionUp)               #listening up