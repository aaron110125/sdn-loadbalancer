...
FLOW_IDLE_TIMEOUT = 10                   #flow controllers
...
class iplb(object):
    ...
    def __init__(self, connection, service_ip, servers=[]):
        self.service_ip = IPAddr(service_ip)                            #server,ip
    ...
    def _pick_server(self, key, inport):
        ...
        if len(self.total_connection) == 0:
            return list(self.live_servers.keys())[0]
        ipserver = list(self.total_connection.keys())[0]
...
def launch(ip, servers):                                             #launching ip server
    ...
    core.openflow.addListenerByName("ConnectionUp", _handle_ConnectionUp)       #listening for connections 