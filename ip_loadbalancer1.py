from pox.core import core                                     #pox controller 
import pox
...
FLOW_IDLE_TIMEOUT = 10                                     #timeout for flows
...
class iplb (object):
  ...
  """
  def __init__ (self, connection, service_ip, servers = []):                    #definining ip, servers, mac
  ...
  def _pick_server (self, key, inport):                       #this will pick your loadbalancer servers
    """
...
def launch (ip, servers):                                         #servers for ip, servers
  ...
  def _handle_ConnectionUp (event):                  #dpid event registered for an ip address, server
    global _dpid
    if _dpid is None:
